...
# Classic accuracy ~ 20 - 40 % should be
...
def compute_metrics(eval_preds):
    preds, labels = eval_preds
    # unpack tuple logits if needed
    if isinstance(preds, tuple):
        preds = preds[0]
    # turn into plain Python lists
    preds = preds.tolist() if hasattr(preds, "tolist") else preds
    labels = labels.tolist() if hasattr(labels, "tolist") else labels
    # clamp any out‐of‐range token IDs
    max_id = tokenizer.vocab_size - 1
    clamped = [[min(max_id, max(0, tok)) for tok in seq] for seq in preds]
    decoded_preds  = tokenizer.batch_decode(clamped, skip_special_tokens=True)
    decoded_labels = tokenizer.batch_decode(labels, skip_special_tokens=True)
    correct = sum(p.strip() == l.strip() for p, l in zip(decoded_preds, decoded_labels))
...
training_args = Seq2SeqTrainingArguments(
    ...
    metric_for_best_model="eval_loss", # criterion of rbest model is the loss on evaluation, small loss -> good model
